    ...
    while (epoch < config.max_epoch):
        ...
        for _, data in enumerate(train_loader, 0):
            ...
            if (config.save_iterations and iteration % config.save_iterations == 0):
                save_model(os.path.join(config.model_dir, 'PRe_PCA6_epoch{}_iter{}.pth'.format(epoch, iteration)), model, optimizer, epoch)
        ...
        # save the model
        if (config.save_epoch and iteration % config.save_epoch == 0):
            save_model(os.path.join(config.model_dir, 'PRe_PCA6_epoch{}.pth'.format(epoch)), model, optimizer, epoch)
            print('-'*80 + '\n{:.2f} h has elapsed'.format((time.time()-start)/3600))
