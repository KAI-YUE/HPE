            ...
            if (files != []):
                ...
                for f in files:
                    if ".dat" in f:
                        ...
                        result, interm = model(Tensor_img)
                        loss = L(result, Tensor_hm, interm)
        ...
        if (files != []):
            ...
            for f in files:
                if ".dat" in f:
                    ...
                    result, interm = model(Tensor_img)
                    loss, _ = L(result[0], interm, result[1], Tensor_hm, Tensor_pos)
...
def Synth_test(Hal, Jor, input_dir, output_dir, device="cuda"):
    """
    Test the joint model on SynthHand dataset. First the Hal model localize the hand image, then 
    the Jor model regresses the joint position.
    --------------------------------------------------------------------------------
    Args,
        Hal,       the hand localization model.
        Jor,       the joint regression model.
...
def Ego_test(Hal, Jor, input_dir, output_dir, device="cuda"):
    """
    Test the joint model on EgoDexter dataset. First the Hal model localize the hand image, then 
    the Jor model regresses the joint position.
    --------------------------------------------------------------------------------
    Args,
        Hal,       the hand localization model.
